...
export default function Indementes() {
    ...
    const fetchIndemnites = async () => {
        try {
            const response = await axiosClient.get('/api/indemnites');
    ...
    const fetchActivities = async (newLimit = 10) => {
        try {
            const response = await axiosClient.get(`/api/activity-logs?limit=${newLimit}`);
    ...
    const handleToggleStatut = async (id) => {
        try {
            await axiosClient.patch(`/api/indemnites/${id}/toggle-statut`);
            setIndemnites(prev => 
                prev.map(item => 
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Êtes-vous sûr de vouloir supprimer cette indemnité")) {
            try {
                await axiosClient.delete(`/api/indemnites/${id}`);
                fetchIndemnites();
                fetchActivities();
            } catch (error) {
    ...
    const handleSave = async () => {
        try {
            if (isEditing) {
                await axiosClient.put(`/api/indemnites/${currentId}`, formData);
            } else {
                await axiosClient.post('/api/indemnites', formData);
            }
