    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-black text-slate-900 tracking-tight">Paramétrage Retraite</h1>
                    <p className="text-slate-500 text-sm font-medium">Gérez le cadre légal et les conditions de fin de carrière.</p>
                </div>
                <button 
                    onClick={handleSave} 
                    disabled={saving}
                    className="flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-lg shadow-indigo-200 transition-all active:scale-95 disabled:opacity-50 cursor-pointer">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                        <div className="flex items-center gap-4 mb-8">
                            ...
                            <div>
                                <h2 className="text-lg font-bold text-slate-800">Seuil de Retraite</h2>
                                <p className="text-xs text-slate-400 font-medium">Définissez l'âge et les alertes préventives.</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="group space-y-2">
                                <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1 group-focus-within:text-indigo-600 transition-colors">
                                ...
                                <div className="relative">
                                    <input 
                                        ...
                                        className={`w-full text-2xl h-16 pl-6 pr-16 rounded-2xl border-2 outline-none font-black transition-all ${
                                            ...
                                            : 'bg-slate-50 border-transparent focus:border-indigo-100 focus:bg-white text-slate-700'
                                        }`}
                                    />
                                    <span className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-400 font-bold">ans</span>
                                </div>
                            </div>

                            <div className="space-y-2">
                                <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">Déclenchement Alerte</label>
                                <div className="relative">
                                    <select 
                                        value={notification} 
                                        onChange={(e) => setNotification(e.target.value)}
                                        className="w-full h-16 px-6 rounded-2xl bg-slate-50 border-2 border-transparent focus:border-indigo-100 focus:bg-white outline-none font-bold text-slate-700 appearance-none cursor-pointer">
                    ...
                    <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm">
                        <div className="flex items-center justify-between mb-8">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h2 className="text-lg font-bold text-slate-800">Politique de Prolongation</h2>
                                    <p className="text-xs text-slate-400 font-medium">Gestion du "Tamdid" après l'âge légal.</p>
                                </div>
                            </div>
                        ...
                        <div className={`space-y-8 transition-all duration-300 ${dureeProlongation > 0 ? 'opacity-100' : 'opacity-40 pointer-events-none grayscale'}`}>
                            <div className="flex items-center justify-between p-5 bg-slate-50 rounded-2xl border border-slate-100">
                                <div>
                                    <span className="block text-sm font-bold text-slate-700">Durée maximale autorisée</span>
                                    <span className="text-[10px] text-slate-400 uppercase font-bold tracking-tighter">Cumul total des années de service supp.</span>
                                </div>
                                <div className="flex items-center gap-3 bg-white p-2 rounded-xl border border-slate-200">
                                    <input 
                                        ...
                                        className="w-12 text-center bg-transparent text-indigo-600 font-black text-lg outline-none"
                                    />
                                    <span className="text-[10px] font-black text-slate-300 uppercase pr-2 border-l pl-2">Ans</span>
                                </div>
                            </div>

                            <div className="space-y-3">
                                <label className="block text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">Fréquence de renouvellement</label>
                                <div className="grid grid-cols-4 gap-3">
                                    {choices.map((opt) => (
                                        <button 
                                            key={opt} 
                                            onClick={() => setNombreProlongations(opt)}
                                            className={`py-3.5 text-xs font-black rounded-2xl border-2 transition-all ${
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-indigo-600 rounded-[2rem] p-8 text-white shadow-xl shadow-indigo-100 relative overflow-hidden group">
                        <div className="relative z-10">
                            <div className="flex items-center gap-2 mb-6">
                                ...
                                <h3 className="font-bold text-sm">Vue d'ensemble</h3>
                            </div>
                    ...
                    <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h3 className="font-bold text-slate-800 text-sm">Règles métier</h3>
                        </div>
                        
                        <div className="space-y-4">
                            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl hover:bg-slate-100/50 transition-colors cursor-pointer group">
                                <span className="text-xs font-bold text-slate-600 group-hover:text-slate-900">Désactiver RCAR</span>
                                <ToggleSwitch checked={desactiverRCAR} onChange={() => setDesactiverRCAR(!desactiverRCAR)} small />
                            </div>
                            
                            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl hover:bg-slate-100/50 transition-colors cursor-pointer group">
                                <span className="text-xs font-bold text-slate-600 group-hover:text-slate-900">Maintenir cotisations</span>
