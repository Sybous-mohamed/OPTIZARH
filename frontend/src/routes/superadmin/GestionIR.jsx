...
const GestionIR = () => {
   ...
   const handleAddYearDirect = async () => {
    ...
    return (
        <div className=" bg-[#FBFCFE] min-h-screen font-sans text-slate-700 p-2">
            {/* Header Section */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-6">
                <div>
                    <h1 className="text-xl font-black text-[#1E293B] tracking-tight flex items-center gap-3">
                        Paramétrage de l'Impôt sur le Revenu(IR)
                        {annee && (
                            <>
                                <ChevronRight className="text-slate-300" size={24}/> 
                                <span className="text-indigo-600">{annee}</span>
                            </>
                        )}
                    </h1>
                ...
                <div className="flex items-center gap-3 bg-white p-2 rounded-[20px] shadow-sm border border-slate-100 focus-within:border-indigo-200 transition-all">
                    <div className="flex items-center gap-2 px-3 border-r border-slate-100">
                        <Calendar size={18} className="text-indigo-500" />
                        <select value={annee || ""} onChange={(e) => setAnnee(e.target.value)}
                            className="bg-transparent font-black text-indigo-600 outline-none cursor-pointer text-sm">
                    ...
                    <div className="flex items-center gap-2 pl-2">
                        <input type="number" placeholder="Nouvel an..."value={newYearInput}
                            ...
                            className="w-28 bg-slate-50 border-none p-2 rounded-xl text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500/10 transition-all"/>
                        <button onClick={handleAddYearDirect}
                            className="bg-indigo-600 text-white p-2.5 rounded-xl hover:bg-indigo-700 transition-all shadow-md shadow-indigo-100 cursor-pointer">
            ...
            {!annee ? (
                <div className="bg-white rounded-[32px] border-2 border-dashed border-slate-100 p-20 flex flex-col items-center justify-center text-center">
                    <div className="bg-indigo-50 p-6 rounded-full mb-6">
                        <AlertCircle size={48} className="text-indigo-500" />
                    </div>
                    <h2 className="text-xl font-bold text-slate-800 mb-2">Aucun exercice sélectionné</h2>
                    <p className="text-slate-400 text-sm max-w-xs">Veuillez choisir une année ou en créer une nouvelle pour modifier le barème.</p>
                </div>
            ) : (
                <>
                    <div className="bg-white rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.02)] border border-slate-100 overflow-hidden relative mb-6">
                        ...
                        <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
                            <div className="flex items-center gap-3 font-bold text-slate-800 text-xs uppercase tracking-widest">
                            ...
                            <div className="flex gap-2">
                                <button onClick={handleDeleteYear}
                                    className="bg-red-50 text-red-500 px-4 py-2 rounded-xl text-[11px] font-black hover:bg-red-500 hover:text-white transition-all flex items-center gap-2 uppercase cursor-pointer">
                                    <Trash size={14}/> Supprimer 
                                </button>
                                <button onClick={addRow}
                                    className="bg-indigo-600 text-white px-5 py-2 rounded-xl text-[11px] font-black hover:bg-indigo-700 transition-all flex items-center gap-2 uppercase shadow-lg shadow-indigo-100 cursor-pointer">
                            ...
                            <table className="w-full border-separate border-spacing-y-2">
                                ...
                                <thead>
                                    <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">
                                        ...
                                        <th className="px-6 py-2 text-emerald-600">Marié </th>
                                        <th className="px-6 py-2 text-emerald-600">Enfant 1</th>
                                        <th className="px-6 py-2 text-emerald-600">Enfant 2</th>
                                ...
                                <tbody>
                                    {rows.map((row, i) => (
                                        <tr key={i} className="bg-slate-50/40 hover:bg-indigo-50/40 transition-all rounded-2xl group">
                                            ...
                                            <td className="px-6 py-3 text-center">
                                                <input 
                                                    ...
                                                    className="w-full bg-transparent text-center font-bold text-emerald-600 outline-none" 
                                                    placeholder="0" />
                                            </td>
                                            <td className="px-6 py-3 text-center">
                                                <input 
                                            ...
                                            <td className="px-6 py-3 text-right last:rounded-r-2xl pr-8">
                                                <button onClick={() => removeRow(i)} 
                                                    className="p-2 text-slate-300 hover:text-red-500 transition-colors  group-hover:opacity-100 cursor-pointer">
                                                    <Trash2 size={16}/>
                                                </button>
                                            </td>
                                        </tr>
                    ...
                    <div className="flex items-center justify-start mb-4 gap-4">
                        ...
                        className="flex items-center gap-2 bg-white border border-slate-200 text-slate-600 px-4 py-3 rounded-xl font-bold text-sm hover:bg-slate-50 transition-all shadow-sm cursor-pointer">
                            <Download size={18}/> Exporter PDF
                        </button>
                        <button onClick={handleSave}disabled={loading}
                            className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-3 rounded-xl font-black text-sm shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95 disabled:opacity-50 cursor-pointer">
